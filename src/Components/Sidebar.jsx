...
function Sidebar() {
  ...
  const [openMenu, setOpenMenu] = useState(() => {
    ...
    if (currentPath.startsWith('/permissions')) return 'permissions';
    ...
    if (currentPath.startsWith('/attributes')) return 'attributes';
    if (currentPath.startsWith('/invoices')) return 'invoices';
    if (currentPath.startsWith('/pages')) return 'pages';
    if (currentPath.startsWith('/authentication')) return 'authentication';
    if (currentPath.startsWith('/base-ui')) return 'baseUi';
    return 'baseUi';
  });
          ...
          {openMenu === 'pages' && (
            <div className="submenu">
              <NavLink
                to="/pages/profile"
                className={({ isActive }) => `submenu-item ${isActive ? 'active' : ''}`}
                onClick={(e) => handleSubItemClick(e, 'pages', '/pages/profile')}
              >
                Profile Page
              </NavLink>
              <NavLink
