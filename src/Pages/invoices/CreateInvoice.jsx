...
function CreateInvoice({ onNavigate }) {
  ...
  const [items, setItems] = useState([
    { id: 1, name: '', size: '', quantity: 1, price: '0.00', tax: '0.00', total: '0.00' },
  ]);

  const [subTotal, setSubTotal] = useState('0.00');
  const [discount, setDiscount] = useState('0.00');
  const [taxAmount, setTaxAmount] = useState('0.00');
  const [grandAmount, setGrandAmount] = useState('0.00');

  const recalculate = (currentItems, currentDiscount = discount) => {
    let sub = 0;
    let totalTax = 0;
    const computedItems = currentItems.map(item => {
      ...
      sub += p * q;
      totalTax += t * q;
      return { ...item, total: tot.toFixed(2) };
    });
    const d = parseFloat(currentDiscount) || 0;
    const grand = Math.max(0, sub + totalTax - d);
    setSubTotal(sub.toFixed(2));
    setTaxAmount(totalTax.toFixed(2));
    setGrandAmount(grand.toFixed(2));
    setAmount(grand.toFixed(2));
  ...
  const handleAddMore = () => {
    const newItems = [
      ...items,
      { id: Date.now(), name: '', size: '', quantity: 1, price: '0.00', tax: '0.00', total: '0.00' },
    ];
    setItems(recalculate(newItems));
  };
  ...
  const handleClearProducts = () => {
    ...
    setItems(recalculate(cleared));
  };

  const handleItemChange = (index, field, value) => {
    const updated = [...items];
    updated[index][field] = value;
    setItems(recalculate(updated));
  };
          ...
          <div className="content-card p-4 mb-4 shadow-sm">
            <div className="row g-3 mb-4">
              ...
              <div className="col-sm-6">
                ...
                <div className="mb-2">
                  <label htmlFor="invStatusSelect" className="form-label text-muted" style={{ fontSize: '0.75rem' }}>Status :</label>
                  <select
                    id="invStatusSelect"
                    ...
                    <tr key={item.id}>
                      <td>
                        <div className="d-flex align-items-center gap-2">
                          <div
                            className="bg-light rounded p-2 text-secondary d-flex align-items-center justify-content-center flex-shrink-0"
                  ...
                  <div className="input-group input-group-sm">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        recalculate(items, e.target.value);
                      }}
                    />
                  </div>
                </div>
